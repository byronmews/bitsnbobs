...
echo -e "sample \t trimmed_input_pairs \t trimmed_pairs_remaining \t trimmed_pairs_remaining_% \t trimmed_forward_only \t trimmed_forward_only_% \t trimmed_reverse_only \t trimmed_reverse_only_% \t trimmed_discarded \t trimmed_discarded_% \t screen_input_pairs \t screen_human_pairs \t screen_human_pairs_% \t screen_univec_pairs \t screen_univec_pairs_% \t pe_reads_passed_qc"
...
for file in *_screen.txt; do
	...
	awk '{if(NR==4) print ($2-$3), ((($2-$3)/($2))*100)}' $file >> temp4_screen_2
	
done

# Count all sequences in final trimmed and screen PE fastq file
for file in *PE_no_hits_file.1.fastq; do
	seqs=`wc -l $file | awk '{print $1/4}'`; echo $seqs >> temp5_seqs;
done
