...
function Help
{
	...
	printf "	\tfastq format: eg. 142_S1_L001_R1_001_PE.*._file.1.fastq\n"
	printf "	\t-a optional split fastqs and run ray assembly (default is skip to motif search)\n"
...
FILE_BASE_NAME=`echo $FASTQ_R1 | sed 's/_L001_R1_001_PE.*.fastq//'`
...
then
	...
	# Assemble binned species PE reads, using RAY_KMER_LEN global var
	printf "Assembling using Ray with kmer length $RAY_KMER_LEN\n"
	for file in taxon_id_*.txt; do
		base=`echo $file | sed 's/taxon_id_//' | sed 's/.txt//'`
		printf "Assembling species taxon_id: $base\n"
		mpiexec -n 7 Ray -k $RAY_KMER_LEN -p "taxon_id_"$base"_R1.fastq" "taxon_id_"$base"_R2.fastq" -output "taxon_id_"$base"_k"$RAY_KMER_LEN"_ray_assembly"
	done
fi
...
then
	printf "\nSkipping fastq split and ray assemblies\n\n"
...
then
	...
	for file in taxon_id_*assembly; do
		fuzznuc -sequence $file"/Contigs.fasta" -pattern NNCGNN -stdout -auto -complement > $file".fuzznuc_nncgnn.out"
	done
fi
...
for file in taxon_id*fuzznuc_nncgnn.out; do
	base=`echo $file | sed 's/_k.*_ray_assembly.*//'`
...
for file in *_ray_assembly/Contigs.fasta; do 
	base=`echo $file | sed 's/_k.*_ray_assembly.*//'` 
...
for file in *fuzznuc_nncgnn.out; do 
	v1=`echo $file | sed 's/_k.*//'`
...
# Temp file 2: atgc base frequencies per species of assmebled contigs
...
printf	"reported_hitcount\t" >> $FILE_BASE_NAME".master.txt"
...
# Moving to more recent scan of all 6-mer motifs
mv $FILE_BASE_NAME".master.txt" $FILE_BASE_NAME".master_225.txt"

# Main tab delim output file
printf "Output table: $FILE_BASE_NAME".master_225.txt""
