...
#
# Fastq QC stages consisting of: FastQC > Trimmomatic_1 > FastQC > Fastq Screen > FLASH  > Trimmomatic_2
#
# Uses multiple configuration files for host screen depending on specifed sample type.
# Used mainly for longer 600bp MiSeq libraries and amplicon 16S sequencing. Joins overlapping PE reads.
#
...
for file_r1 in *R1_001.fastq.gz; do
	...
	# Contaminant list originates from fastq library, with any solid  platform primers removed from list. Prob overkill for the projects though. Hexamer bias from 5' end kept in this trimming round. No trimming within the sequence either.
